...
public class MovieAndTVShowRepository implements MovieAndTVShowDataSource{
    private volatile static MovieAndTVShowRepository INSTANCE = null;
    private final RemoteRepository remoteRepository;
    private final LocalRepository localRepository;
    private final AppExecutors appExecutors;

    private MovieAndTVShowRepository(@NonNull RemoteRepository remoteRepository, @NonNull LocalRepository localRepository, AppExecutors appExecutors) {
        this.remoteRepository = remoteRepository;
        this.localRepository = localRepository;
        this.appExecutors = appExecutors;
    }

    public static MovieAndTVShowRepository getInstance(RemoteRepository remoteRepository,LocalRepository localRepository, AppExecutors appExecutors) {
        if (INSTANCE == null) {
            synchronized (MovieAndTVShowRepository.class) {
                if (INSTANCE == null) {
                    INSTANCE = new MovieAndTVShowRepository(remoteRepository, localRepository, appExecutors);
                }
            }
        }
        return INSTANCE;
    ...
    public LiveData<Resource<List<MovieEntity>>> getListMovies() {
        return new NetworkBoundResource<List<MovieEntity>, List<MovieResponse>>(appExecutors) {
            ...
            @Override
            protected void saveCallResult(List<MovieResponse> listMoviesReponse) {
                List<MovieEntity> listMovies = new ArrayList<>();
                for (MovieResponse movieResponse : listMoviesReponse) {
                ...
                localRepository.insertListMovies(listMovies);
            }
        }.asLiveData();
    }
    ...
    public LiveData<Resource<List<TVShowEntity>>> getListTVShow() {
        return new NetworkBoundResource<List<TVShowEntity>, List<TVShowResponse>>(appExecutors) {
            ...
            @Override
            protected void saveCallResult(List<TVShowResponse> listTVShowResponse) {
                List<TVShowEntity> listTVSHow = new ArrayList<>();

                for (TVShowResponse tvShowResponse : listTVShowResponse) {
                    listTVSHow.add(new TVShowEntity(
                ...
                localRepository.insertListTVShow(listTVSHow);
            }
        }.asLiveData();
    }
    ...
    public void setFavoriteMovie(MovieEntity movieEntity, boolean state) {
        Runnable runnable = () -> localRepository.setFavoriteMovie(movieEntity, state);
        appExecutors.diskIO().execute(runnable);
    }

    @Override
    public void setFavoriteTVShow(TVShowEntity tvShowEntity, boolean state) {
        Runnable runnable = () -> localRepository.setFavoriteTVShow(tvShowEntity, state);
        appExecutors.diskIO().execute(runnable);
    }
